...
- La función `is_admin(uid)` consulta `public.perfiles.is_admin`.
 - Incluye un **seed** de categorías iniciales (si la tabla `categories` está vacía). Es seguro re-ejecutarlo.
...
### Requerida para administrar usuarios (backend)
- `SUPABASE_SERVICE_ROLE_KEY`

> Esta clave **solo** se configura en el servidor (Vercel) y no debe exponerse en el cliente.

> En Supabase, el **publishable key** normalmente corresponde al “anon key”.

### Requerida para administración de usuarios (Créditos) 
Para activar usuarios o reenviar correos desde el módulo Créditos, el backend usa Admin API.

- `SUPABASE_SERVICE_ROLE_KEY` (solo servidor; en Vercel agrégala como env var, no es NEXT_PUBLIC)
...
- Si la tabla `categories` está vacía, el panel `/admin` insertará automáticamente categorías iniciales (las mismas del `schema.sql`).
