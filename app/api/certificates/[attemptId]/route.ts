...
export async function GET(
  ...
  const { data: cert } = await supabase
    .from("certificates")
    .select("folio_code,verify_code,issued_at,settings_snapshot")
  ...
  // settings_snapshot proviene de la tabla certificate_settings serializada; usamos campos snake_case.
  ...
  ].filter((a) => a?.name || a?.title || a?.signatureUrl) as any;
  ...
  const element = CertificateDoc({
    ...
    folio: (cert as any).folio_code ?? null,
    authorities,
    logoUrl,
    watermarkUrl,
  });
