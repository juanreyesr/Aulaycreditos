...
function withTimeout<T>(promise: Promise<T>, ms: number, label = "Operación") {
  return Promise.race([
    ...
    new Promise<T>((_, reject) =>
      setTimeout(() => reject(new Error(`${label}: tiempo de espera (${ms}ms).`)), ms)
    ),
  ]);
}
...
const CategorySchema = z.object({
  ...
  // Cuando el input está vacío, llega "" (no undefined). Lo normalizamos para evitar problemas.
...
const CourseSchema = z.object({
  ...
  // Nota: el <select> manda "" cuando no se elige nada.
  category_id: z.preprocess(emptyToUndefined, z.string().uuid().optional()),
  // Nota: el usuario suele pegar links sin https://; los normalizamos.
  youtube_url: z.preprocess(
    normalizeUrlOrUndefined,
    z
      .string()
      .url("URL inválida")
      .refine((u) => u.startsWith("https://"), "Usa un enlace https://")
      .optional()
  ),
  // Next/Image está configurado para permitir solo imágenes remotas https.
...
setCertificates((certs ?? []) as any[]);
  ...
  async function ensureDefaultCategories() {
    // Asegura que existan las categorías iniciales.
    // Importante: NO depende de que la tabla esté vacía, porque el usuario puede haber
    // creado una o dos categorías manualmente (y en ese caso igual queremos insertar
    // las faltantes).
    try {
      const jwt = (await supabase.auth.getSession()).data.session?.access_token;

      // 1) Intento preferido: endpoint backend (service role) para evitar problemas de RLS.
      if (jwt) {
        const res = await fetch("/api/admin-seed-categories", {
          method: "POST",
          headers: { Authorization: `Bearer ${jwt}` },
        });
        if (res.ok) return; // ya quedó sembrado
      }

      // 2) Fallback (cliente): inserta solo las faltantes (sin depender de UNIQUE en slug).
      const { data: existing } = await supabase.from("categories").select("name,slug");
      ...
      // Silencioso: no bloqueamos el panel.
    }
  }
  ...
  async function addCategory() {
    ...
    try {
      ...
      const base = (parsed.slug?.trim() ? parsed.slug!.trim() : parsed.name).toString();
      const slug = slugify(base) || null;
      ...
      if (e instanceof z.ZodError) {
        setMsg(e.issues?.[0]?.message ?? "Datos inválidos.");
      } else {
        const extra = [e?.details, e?.hint, e?.code].filter(Boolean).join(" | ");
        setMsg(`${e?.message ?? "No se pudo crear la categoría."}${extra ? ` (${extra})` : ""}`);
      }
    } finally {
      setBusy(false);
    }
  ...
  async function addCourse() {
    ...
    try {
      ...
      if (parsed.youtube_url && !youtube_video_id) {
        throw new Error(
          "No pude identificar el ID del video de YouTube. Usa un enlace completo tipo https://youtu.be/ID o https://www.youtube.com/watch?v=ID"
        );
      }
      const payload = {
      ...
      // 1) Preferido: endpoint backend (service role) para evitar bloqueos por RLS.
      const jwt = (await supabase.auth.getSession()).data.session?.access_token;
      if (jwt) {
        ...
        try {
          ...
          if (res.ok && data?.ok) {
            ...
            await withTimeout(refresh(), 15000, "Refresco");
            setMsg("Video/curso creado.");
            return;
          }
          // Si falla, guardamos el error para mostrarlo si el fallback también falla.
          const extra = [data?.details, data?.hint, data?.code]
            .filter(Boolean)
            .join(" | ");
          backendError = data?.error ? String(data.error) : `HTTP ${res.status}`;
          if (extra) backendError += ` (${extra})`;
        } catch {
          clearTimeout(t);
          // fallback
        }
      ...
      // 2) Fallback: inserción directa (requiere políticas/RLS correctas)
      ...
      if (e instanceof z.ZodError) {
        ...
        const extra = [e?.details, e?.hint, e?.code].filter(Boolean).join(" | ");
        const base = `${e?.message ?? "No se pudo crear el video/curso."}${extra ? ` (${extra})` : ""}`;
        setMsg(backendError ? `${base} | Backend: ${backendError}` : base);
      }
    } finally {
      setBusy(false);
    }
          ...
          <div className="rounded-2xl border border-white/10 bg-black/30 p-6">
            ...
            <div className="mt-4 grid gap-3">
              ...
              <button disabled={busy} onClick={addCourse} className="rounded-full bg-cpgRed px-5 py-2 text-sm hover:opacity-90 disabled:opacity-60">
                {busy ? "Guardando..." : "Crear video/curso"}
              </button>
...
{tab === "security" && (
        <div className="mt-6 rounded-2xl border border-white/10 bg-black/30 p-6">
          ...
          <ul className="mt-3 list-disc pl-5 text-sm text-white/70 space-y-2">
            <li>
              No se incluyen contraseñas “hardcodeadas” en el repositorio. Los administradores se gestionan en Supabase Auth y mediante <code>public.perfiles.is_admin = true</code>.
            </li>
            <li>
