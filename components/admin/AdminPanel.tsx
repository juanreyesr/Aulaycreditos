...
setCertificates((certs ?? []) as any[]);
  ...
  async function ensureDefaultCategories() {
    ...
    // Se ejecuta solo al cargar el panel (y solo si no hay registros).
    const { data: anyCat, error } = await supabase.from("categories").select("id").limit(1);
    if (error) return;
    if ((anyCat ?? []).length > 0) return;

    const payload = DEFAULT_CATEGORIES.map((name, idx) => ({
      name,
      slug: slugify(name),
      sort_order: idx + 1,
    }));
