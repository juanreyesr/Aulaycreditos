...
// En esta versión se usa un endpoint de Next.js (/api/admin-users) para evitar depender de Edge Functions
// y poder manejar el Service Role de forma segura en el servidor.
async function callAdminUserFunction(action, email){
  ...
  if(!token) throw new Error('Sesión inválida. Inicie sesión como superadmin.');
...
userResendBtn?.addEventListener('click', async ()=>{
  ...
  try {
    ...
    showToast('No se pudo reenviar: ' + sbErrMsg(e), 'error');
  }
});
...
userActivateBtn?.addEventListener('click', async ()=>{
  ...
  try {
    ...
    if(userAdminState) userAdminState.textContent = 'Error: ' + sbErrMsg(e);
    showToast('No se pudo activar: ' + sbErrMsg(e), 'error');
  }
});
