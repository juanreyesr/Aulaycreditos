...
-- IMPORTANTE: Para este proyecto integrado, la autoridad de administración
-- se toma desde la tabla `public.perfiles` del sistema de Créditos Académicos.
-- Si `perfiles` ya existe en tu proyecto, esta sección NO lo rompe.
...
-- Garantiza compatibilidad (si la tabla existía con otra estructura)
alter table public.perfiles add column if not exists is_admin boolean not null default false;
...
-- Categorías iniciales (puedes volver a ejecutar este archivo; no duplica por el ON CONFLICT)
insert into public.categories (name, slug, sort_order)
values
  ('Psicología clínica', 'psicologia-clinica', 1),
  ('Neurocientífico', 'neurocientifico', 2),
  ('Educativa', 'educativa', 3),
  ('Deportiva', 'deportiva', 4),
  ('Psicología social', 'psicologia-social', 5),
  ('Psicometría', 'psicometria', 6)
...
-- Nota: No modificamos políticas/RLS de `public.perfiles` para no interferir con el módulo
-- de Créditos Académicos ya existente. La función is_admin(...) consulta `perfiles`.
...
returns boolean as $$
  select coalesce(
    (select p.is_admin from public.perfiles p where p.user_id = uid limit 1),
    false
  );
$$ language sql stable;
